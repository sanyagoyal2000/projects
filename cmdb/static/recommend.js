const API_KEY = '34277c155e8341d536807380395f869a'

// button will be disabled until we type anything in the input field
// and calls load_details() if str is present
$(function() {
    const source = document.getElementById('autoComplete');
    $('.movie-button').attr('disabled', true); //disabled
    const inputHandler = function(e) {
        if (e.target.value != "") {
            $('.movie-button').attr('disabled', false); //enabled
        } else {
            $('.movie-button').attr('disabled', true); //disabled
        }
    }
...
function recommendcard(e) {
    ...
    var title_parsed = title.replace(/'/g, '"')
    var t = JSON.parse(title_parsed)
    load_details(my_api_key, t[0]);
}

// get the basic details of the movie from the API (based on the name of the movie)
function load_details(my_api_key, title) {
    console.log("calling load_details()")
    ...
    $.ajax({
        ...
        success: function(movie) {
            if (movie.results.length < 1) {
                ...
                $('.results').delay(1000).css('display', 'block') // didn't understand
...
function movie_recs(movie_title, movie_id, my_api_key) {
    console.log("calling movie_recs()")
    $.ajax({
        type: "POST",
        url: "/similarity",
        data: { 'name': movie_title },
        success: function(recs) {
            if (recs == 'Sorry! Movie you searched is not available in the database') {
                $('.fail').css('display', 'block');
                $('.results').css('display', 'none');
                $("#loader").delay(500).fadeOut();
            } else {
...
// passing all th details to python's flask for displaying and scripting the movie reviews using imdb id
